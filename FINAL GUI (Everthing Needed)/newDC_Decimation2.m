...
% BELOW TWO FUNCTION RELATED TO THE OSR VALUE IN THE CONFIG PART
...
% BELOW TWO FUNCTIONS RELATED TO THE FOUT VALUE IN THE CONFIG PART 
...
% BELOW TWO FUNCTIONS RELATED TO THE ENOB VALUE IN THE CONFIG PART
...
% CALIBRATION FUNCTION CAN DIRECTLY RECALCULATED THE P_LEFT AND P_RIGHT
% In this lab, we used the p_left and p_right value by manually calculating
% it will be helpful in the future we have this function automatically
% calibrate those value and set them in the next function
function Calibration_Callback(hObject, eventdata, handles)




% BY RUNNING THIS FUNCTION, WE CONGIF THE FPGA(READY TO WORK) AND GENERATE
% ALL NEEDED PLOT
function confirmConfiguration_Callback(hObject, eventdata, handles)

% config the FPGA, and make it ready to work. 
...
% generate the muilti-tap plot, I used the uitabgroup   
handles.axes_total=[]; 
f = figure('Name','FSCV GUI','NumberTitle','off','Color','w','Position',[350 50 1550 935]); 
tg = uitabgroup(f);


% generate the first tab, with six plots on it
...
% generate the second tab with six plots on it
...
% generate the third tab with six plots on it 
thirdTab = uitab(tg,'Title', 'Ch1 & Ch2 Realtime Measurement');
 ...
 % generate the fouth tab with six plots on it 
...
% setup the p_left and p_right values. Note* we got these values by
% manurally calibrate. It will be great in the future we can implement a
% function to aumatically calculte them and reset them below. 
...
% setup the feedback resistants value for four different TIAs
...
% THIS FUNCTION WILL RESET THE FPGA
...
% THIS FUNCTION WILL UNLOAD THE FPGA
...
% THIS FUNCTION WILL ENABLE THE FPGA
...
% THE NEXT TWO FUNCTION WILL ALLOW USER TO CHOOSE THE VALUE OF 'dB' IN PGA FOR
% CHANNLE 4
...
% THIS FUNCTION WILL ALLOW USER TO ENABLE USING THE CHANNEL 4
...
% THE NEXT TWO FUNCTIONS WILL WHAT THEY WANT TO MEASURE (VOLTAGE OR
...
% THIS FUNCTION WILL ALLOW USER TO ENABLE USING THE CHANNEL 3
...
% THIS FUNCTION WILL ALLOW USER TO ENABLE USING THE CHANNEL 2
...
% THIS FUNCTION WILL ALLOW USER TO ENABLE USING THE CHANNEL 1
...
% THE FOLLOWING FUNCTION ALLOW USER TO SETUP THE PEAK VOL OF CHANNEL4
...
% THE FOLLOWING FUNCTION ALLOW USER TO SETUP THE BOTTOM VOL OF CHANNEL4
...
% THE FOLLOWING FUNCTION ALLOW USER TO SETUP THE Vcom OF CHANNEL4
...
% THE FOLLOWING FUNCTION ALLOW USER TO SETUP THE PEAK VOL OF CHANNEL3
...
% THE FOLLOWING FUNCTION ALLOW USER TO SETUP THE BOTTOM VOL OF CHANNEL3
...
% THE FOLLOWING FUNCTION ALLOW USER TO SETUP THE Vcom OF CHANNEL3
...
% THE FOLLOWING FUNCTION ALLOW USER TO SETUP THE PEAK VOL OF CHANNEL2
...
% THE FOLLOWING FUNCTION ALLOW USER TO SETUP THE BOTTOM VOL OF CHANNEL2
...
% THE FOLLOWING FUNCTION ALLOW USER TO SETUP THE Vcom OF CHANNEL2
...
% THE FOLLOWING FUNCTION ALLOW USER TO SETUP THE PEAK VOL OF CHANNEL1
...
% THE FOLLOWING FUNCTION ALLOW USER TO SETUP THE BOTTOM VOL OF CHANNEL1
...
% THE FOLLOWING FUNCTION ALLOW USER TO SETUP THE Vcom OF CHANNEL1
...
% THE FOLLOWING FUNCTION ALLOW USER TO SET UP THE FREQUENCY VALUE IN REAL
% MEASUREMENT (*NOTE IT IS NOT THE FREQUENCY VALUE FOR PRETREATMENT)
...
% THE FOLLOWING FUNCTION ALLOW USER TO SET UP THE SCANRATE VALUE IN REAL
% MEASUREMENT (*NOTE IT IS NOT THE SCANRATE VALUE FOR PRETREATMENT)
...
% THE FOLLOWING FUNCTION ALLOW USER TO START THE REAL MEASUREMENT
function MeasureStart_Callback(hObject, eventdata, handles)

% clear the previor plot
...
% setup the needed parameter based on related value in the edit boxes
...
% generate the array for storing and ploting data
handles.data_L1=[];
handles.data_R1=[];
handles.current_L1=[];
handles.current_R1=[];
...
% generate the data array for further saving purpose
...
% enable the FPGA to set the wire in value
...
% start measuring, data fetching and realtime plotting
for i= 1:cycle
    pause(delay-t);
    tic
    
    % get the necessary boundary vol value for arranging the axis
    ...
    % starting fetching initial data
    ...
    % data calculted and store in related arraies
    ...
    % realtime the calculated data to related plot
    ...
    % calculate the FSCV data based on measured voltage and current value
    % and store them in related arraies
%     [data_FSCV_L1, N_L1]=FSCV_cycle_v2(data_cycle_L1, handles.Fout, Period, ScanRate, ch1Vr_H, ch1Vr_L, tolerance_range, N_ok, handles.p_Left1);
    ...
    % calculate the FSCV (current) data based on the above FSCV data
    ...
    % calculate the FSCV (voltage) data based on the above FSCV data
%     Xaxes_L1=[(((0:(N_L1-1))*((ch1Vr_H-ch1Vr_L)/N_L1))+ch1Vr_L) ch1Vr_H ((((N_L1-1):(-1):0)*((ch1Vr_H-ch1Vr_L)/N_L1))+ch1Vr_L)];
    ...
    % plot the FSCV (V VS I) based on above calculation
    plot(handles.axes_total(18),Xaxes_R1,imp_R1*(10^9))
    axis(handles.axes_total(18),[ch2lowerVol ch2upperVol ch2lowerCur ch2upperCur])
    ...
    axis(handles.axes_total(21),[ch3lowerVol ch3upperVol ch3lowerCur ch3upperCur])
    ...
    axis(handles.axes_total(24),[ch4lowerVol ch4upperVol ch4lowerCur ch4upperCur])
    ...
    % for each 15 loops, we update the cursor value
    ifmod3 = mod(i,15);
    if(ifmod3 == 0)

        % redefine the FSCV voltage data
    ...
    %     ch1I4b = imp_L1(ch1vol4idx);
        ...
        % set the calculated current value to the related edit boxes 
...
% THIS FUNCTION IS DESIGNED FOR THE STOP PURPOSE, SINCE WE SETUP CERTAIN
% AMONT TIME FOR MEASUREMENT WE DONT NEED IT. BUT IT IS NECESSARY FOR
% FUTURE IMPLEMENT
function togglebutton4_Callback(hObject, eventdata, handles)



% THIS IS THE MANUALLY SAVE FILE FUNCTION FOR REAL MEASUREMENT
...
% THE NEXT TWO FUNCTIONS ALLOW US TO SETUP HOW LONG FOR EACH MEARUEMENT. 
...
% THE NEXT TWO FUNCTION ALLOW US TO SETUP THE FREQUENCY VALUE FOR
% PRETREATMENT
...
% THE NEXT TWO FUNCTIONS ALLOW US TO SETUP THE SCANRATE VALUE FOR
% PRETREATMENT 
...
% THIS FUNCTION ALLOWS USERS TO START THE PRETREATMENT (*Note THIS FUNCTION IS SIMILIAR AS THE REAL MEASURMENT START FUNCTION)
function PretreatStart_Callback(hObject, eventdata, handles)

% clear any caches on plots
...
% setup all necessary parameters based on users preferance
...
% generate the array to store the cursor (current) values 
ch1cursorCur = [];
ch2cursorCur = [];
ch3cursorCur = [];
ch4cursorCur = [];

% generate the array to store the slope of cursor values 
ch1CurSlop = [];
ch2CurSlop = [];
ch3CurSlop = [];
ch4CurSlop = [];
...
for i= 1:cycle
    ...
    % since the pretreatment takes long time, we need to clear cache every
    % 100 loops
    ifmod2 = mod(i,100);
    if(ifmod2 == 0)
    ...
    % calculate and store the initial data into related arraies
    ...
    data_tmp_R1 = (data_tmp_R1-handles.p_Right1(2))/(handles.p_Right1(1));
    handles.data_R1=[handles.data_R1; data_tmp_R1];
    current_tmp_R1=(data_tmp_R1-ch2Vcom)/(RfeedBack(2));
    handles.current_R1=[handles.current_R1; current_tmp_R1];
    data_cycle_R1=[data_cycle_R1; data_tmp_R1];
    ...
    % plot the calculated voltage and current data vs time
    ...
    % since calculate the FSCV data is computationally costly, we did this
    % for every 15 loops
    ifmod = mod(i,15);
    if(ifmod == 0)
        
        % calculate the initial FSCV data, based on previous data
        [data_FSCV_L1, N_L1]=FSCV_cycle_v2(data_cycle_L1, handles.Fout, Period, ScanRate, ch1Vr_H, ch1Vr_L, tolerance_range, N_ok, handles.p_Left1);
        [data_FSCV_R1, N_R1]=FSCV_cycle_v2(data_cycle_R1, handles.Fout, Period, ScanRate, ch2Vr_H, ch2Vr_L, tolerance_range, N_ok, handles.p_Right1);
        [data_FSCV_L2, N_L2]=FSCV_cycle_v2(data_cycle_L2, handles.Fout, Period, ScanRate, ch3Vr_H, ch3Vr_L, tolerance_range, N_ok, handles.p_Left2);
        [data_FSCV_R2, N_R2]=FSCV_cycle_v2(data_cycle_R2, handles.Fout, Period, ScanRate, ch4Vr_H, ch4Vr_L, tolerance_range, N_ok, handles.p_Right2);
        
        % calculate the FSCV (current) data 
        ...
        % calculate the FSCV (voltage) data
        Xaxes_L1=[(((0:(N_L1-1))*((ch1Vr_H-ch1Vr_L)/N_L1))+ch1Vr_L) ch1Vr_H ((((N_L1-1):(-1):0)*((ch1Vr_H-ch1Vr_L)/N_L1))+ch1Vr_L)];
        Xaxes_R1=[(((0:(N_R1-1))*((ch2Vr_H-ch2Vr_L)/N_R1))+ch2Vr_L) ch2Vr_H ((((N_R1-1):(-1):0)*((ch2Vr_H-ch2Vr_L)/N_R1))+ch2Vr_L)];
        Xaxes_L2=[(((0:(N_L2-1))*((ch3Vr_H-ch3Vr_L)/N_L2))+ch3Vr_L) ch3Vr_H ((((N_L2-1):(-1):0)*((ch3Vr_H-ch3Vr_L)/N_L2))+ch3Vr_L)];
        Xaxes_R2=[(((0:(N_R2-1))*((ch4Vr_H-ch4Vr_L)/N_R2))+ch4Vr_L) ch4Vr_H ((((N_R2-1):(-1):0)*((ch4Vr_H-ch4Vr_L)/N_R2))+ch4Vr_L)];
        ...
        % plot the FSCV (voltage vs current) to the corresponding plots 
        ...
        axis(handles.axes_total(6),[ch2lowerVol ch2upperVol ch2lowerCur ch2upperCur])
        ...
        axis(handles.axes_total(9),[ch3lowerVol ch3upperVol ch3lowerCur ch3upperCur])
        ...
        axis(handles.axes_total(12),[ch4lowerVol ch4upperVol ch4lowerCur ch4upperCur])
        ...
        % from here to the end of the if statment, we did the same job to
        % calculate the cursor (current) value by given cursor (voltage)
        % and disply them on the GUI editboxes
        ch1voltageArray = Xaxes_L1;
        ch2voltageArray = Xaxes_R1;
        ch3voltageArray = Xaxes_L2;
        ch4voltageArray = Xaxes_R2;
...
% THIS FUNCTION WILL ALLOW USER TO CHOOSE DIRECTOR FOR DATA SAVING
...
% THIS FUNCTION WILL HELP USER TO AUTOMATICALLY GENERATE THE FILE NAME
% (BASED ON NEEDED PARAMETERS)
...
% THIS FUNCTION WILL ALLOW USER TO CHANGE THE PRETREATMENT DEVICE ID
...
% THIS FUNCTION WILL DISPLAY THE DIRECTORY FOR STORING PRETREATMENT DATA
...
% THIS FUNCTION WILL ALLOW USERS TO CHOOSE WHICH CHANNEL TO DISPLAY THE
% SLOPE VALUE
...
% THESE TWO FUNCTION WILL DISPLAY THE SLOPE VALUE CORRESPOND TO THE
% I(CURRENT)1 
...
% THESE TWO FUNCTIONS DETERMINE THE LOWER VOLTAGE VALUE FOR DISPLAY FSCV OF
% CHANNEL4
...
% THESE TWO FUNCTIONS DETERMINE THE LOWER CURRENT VALUE FOR DISPLAY FSCV OF
% CHANNEL4
...
% THESE TWO FUNCTIONS DETERMINE THE UPPER VOLTAGE VALUE FOR DISPLAY FSCV OF
% CHANNEL4
...
% THESE TWO FUNCTIONS DETERMINE THE UPPER CURRENT VALUE FOR DISPLAY FSCV OF
% CHANNEL4
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V1 OF CHANNEL 4
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V2 OF CHANNEL 4
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V3 OF CHANNEL 4
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V4 OF CHANNEL 4
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I1 OF CHANNEL4
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I2 OF CHANNEL4
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I3 OF CHANNEL4
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I4 OF CHANNEL4
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V1 OF CHANNEL 3
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V2 OF CHANNEL 3
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V3 OF CHANNEL 3
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V4 OF CHANNEL 3
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I1 OF CHANNEL3
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I2 OF CHANNEL3
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I3 OF CHANNEL3
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I4 OF CHANNEL3
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V1 OF CHANNEL 2
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V2 OF CHANNEL 2
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V3 OF CHANNEL 2
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V4 OF CHANNEL 2
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I1 OF CHANNEL2
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I2 OF CHANNEL2
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I3 OF CHANNEL2
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I4 OF CHANNEL2
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V1 OF CHANNEL 1
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V2 OF CHANNEL 1
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V3 OF CHANNEL 1
...
% THESE TWO FUNCTION ALLOW USER TO DEFINE THE CURSOR V4 OF CHANNEL 1
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I1 OF CHANNEL1
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I2 OF CHANNEL1
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I3 OF CHANNEL1
...
% THERE TWO FUNCTION ALLOW TO DISPLAY THE MEASURED CURRENT I4 OF CHANNEL1
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL4 V1 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL4 V2 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL4 V3 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL4 V4 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I1 VALUE BASED ON CHANNEL4 CURSOR
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I2 VALUE BASED ON CHANNEL4 CURSOR
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I3 VALUE BASED ON CHANNEL4 CURSOR
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I4 VALUE BASED ON CHANNEL4 CURSOR
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL3 V1 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL3 V2 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL3 V3 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL3 V4 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I1 VALUE BASED ON CHANNEL3 CURSOR
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I2 VALUE BASED ON CHANNEL3 CURSOR
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I3 VALUE BASED ON CHANNEL3 CURSOR
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I4 VALUE BASED ON CHANNEL3 CURSOR
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL2 V1 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL2 V2 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL2 V3 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL2 V4 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I1 VALUE BASED ON CHANNEL2 CURSOR
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I2 VALUE BASED ON CHANNEL2 CURSOR
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I3 VALUE BASED ON CHANNEL2 CURSOR
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I4 VALUE BASED ON CHANNEL2 CURSOR
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL1 V1 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL1 V2 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL1 V3 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW USER SETTING THE CHANNEL1 V4 CURSOR (PRETREATMENT)
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I1 VALUE BASED ON CHANNEL1 CURSOR
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I2 VALUE BASED ON CHANNEL1 CURSOR
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I3 VALUE BASED ON CHANNEL1 CURSOR
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE I4 VALUE BASED ON CHANNEL1 CURSOR
...
% THIS FUNCTION ALLOW USER TO CHOOSE DIRECTORY FOR STORING REAL
% MEASUREMENT DATA
...
% THIS FUNCTION HELPS TO AUTOMATICALLY GENERATE THE FILE NAME
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE FILE NAME FOR REAL MEASUREMENT
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE DEVICE NAME FOR REAL MEASUREMENT
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE MOLECULE INFOMATION FOR REAL
% MEASUREMENT
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE CONCENTRATION FOR REAL MEASUREMENT
...
% THESE FUNCTIONS ALLOW TO DISPLAY THE DIRECTORY ADDRESS FOR STORING THE
% FILES
...
% THIS FUNCTION HELPS TO DO DATA ANALYSIS FOR THE CHANNEL 1&2 WHILE THE
% INPUT SIGNAL IS N-SHAPE
...
% THIS FUNCTION HELPS TO DO DATA ANALYSIS FOR THE CHANNEL 3&4 WHILE THE
% INPUT SIGNAL IS N-SHAPE
...
% THIS FUNCTION HELPS TO DO DATA ANALYSIS FOR THE CHANNEL 1 WHILE THE
...
% THIS FUNCTION HELPS TO DO DATA ANALYSIS FOR THE CHANNEL 2 WHILE THE
% INPUT IS RAMP SIGNAL
...
% THIS FUNCTION HELPS TO DO DATA ANALYSIS FOR THE CHANNEL 3 WHILE THE
% INPUT IS RAMP SIGNAL
...
% THIS FUNCTION HELPS TO DO DATA ANALYSIS FOR THE CHANNEL 4 WHILE THE
% INPUT IS RAMP SIGNAL
...
% THESE FUNCTIONS ALLOW USERS TO DETERMINE THE PRETREATMENT DURATION
...
% THESE FUNCTIONS ALLOW USERS TO DETERMINE THE STARTING CYCLE FOR DATA
% ANALYSIS WHEN THE INPUT IS RAMP SIGNAL 
...
% THESE FUNCTIONS ALLOW USERS TO DETERMINE THE ENDING CYCLE FOR DATA
% ANALYSIS WHEN THE INPUT IS RAMP SIGNAL 
...
% THESE FUNCTIONS ALLOW USERS TO DETERMINE THE STARTING CYCLE FOR DATA
% ANALYSIS (DOPAMINE) WHEN THE INPUT IS N-SHAPE 
...
% THESE FUNCTIONS ALLOW USERS TO DETERMINE THE STARTING CYCLE FOR DATA
% ANALYSIS (PBS) WHEN THE INPUT IS N-SHAPE 
