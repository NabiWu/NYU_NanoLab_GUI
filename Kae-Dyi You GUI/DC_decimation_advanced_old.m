...
% Last Modified by GUIDE v2.5 05-Apr-2019 20:01:34
...
handles.axes_total=[]; 
figure('Name','FSCV GUI','NumberTitle','off','Color','w','Position',[350 50 1550 935]); 
tmp=subplot(4,3,1); 
title('t vs v (AinL)','Color', 'b') 
xlabel('time (s)') 
ylabel('voltage (V)') 
handles.axes_total=[handles.axes_total tmp]; 

tmp=subplot(4,3,4); 
title('t vs code (AinL)','Color', 'b') 
xlabel('time (s)') 
ylabel('Code Value (quarter bit)') 
handles.axes_total=[handles.axes_total tmp]; 

tmp=subplot(4,3,2); 
title('t vs v (TIAXX L)','Color', 'b') 
xlabel('time (s)') 
ylabel('voltage (V)') 
handles.axes_total=[handles.axes_total tmp]; 

tmp=subplot(4,3,5); 
title('t vs i (TIAXX L)','Color', 'b') 
xlabel('time (s)') 
ylabel('Current (nA)') 
handles.axes_total=[handles.axes_total tmp]; 

tmp=subplot(4,3,[3 6]); 
title('v vs i (TIAXX L)','Color', 'b') 
xlabel('Vcm voltage (v)') 
ylabel('Current (nA)') 
handles.axes_total=[handles.axes_total tmp]; 

tmp=subplot(4,3,7); 
title('t vs v (AinR)','Color', 'b') 
xlabel('time (s)') 
ylabel('voltage (V)') 
handles.axes_total=[handles.axes_total tmp]; 

tmp=subplot(4,3,10); 
title('t vs code (AinR)','Color', 'b') 
xlabel('time (s)') 
ylabel('Code Value (quarter bit)') 
handles.axes_total=[handles.axes_total tmp]; 

tmp=subplot(4,3,8); 
title('t vs v (TIAXX R)','Color', 'b') 
xlabel('time (s)') 
ylabel('voltage (V)') 
handles.axes_total=[handles.axes_total tmp]; 

tmp=subplot(4,3,11); 
title('t vs i (TIAXX R)','Color', 'b') 
xlabel('time (s)') 
ylabel('Current (nA)') 
handles.axes_total=[handles.axes_total tmp]; 

tmp=subplot(4,3,[9 12]); 
title('v vs i (TIAXX R)','Color', 'b') 
xlabel('Vcm voltage (v)') 
ylabel('Current (nA)') 
handles.axes_total=[handles.axes_total tmp];
...
handles.RfeedBack_TIA2L=4.1*(10^6);
handles.RfeedBack_TIA2R=4.1*(10^6);
handles.RfeedBack_TIA1L=660*(10^3);
handles.RfeedBack_TIA1R=660*(10^3);
...
%tmp0 = 0;
%tmp(cycle,1) = 0;
data_tmp_L((FIFO_size-1),1) = 0;
data_tmp_R((FIFO_size-1),1) = 0;
...
for i= 1:cycle
    ...
    data_tmp_L = quantizer_one_forth((double(epvalue_A0(1:2:(2*(FIFO_size-1)-1)))+256*double(epvalue_A0(2:2:(2*(FIFO_size-1)))))/(2^(16-ENOB)));
    data_tmp_L=(data_tmp_L-handles.p_Left(2))/handles.p_Left(1);
    handles.data_L=[handles.data_L; data_tmp_L];
    data_tmp_R = quantizer_one_forth((double(epvalue_A1(1:2:(2*(FIFO_size-1)-1)))+256*double(epvalue_A1(2:2:(2*(FIFO_size-1)))))/(2^(16-ENOB)));
    data_tmp_R=(data_tmp_R-handles.p_Right(2))/handles.p_Right(1);
    handles.data_R=[handles.data_R; data_tmp_R];
    tmp0 = round(((i-1)-mod((i-1),N_window))/N_window)+1;
    plot(handles.axes_total(1),((((i-1)*(FIFO_size-1)):1:(i*(FIFO_size-1)-1))/handles.Fout),data_tmp_L)
    axis(handles.axes_total(1),[((tmp0-1)*(N_window*(FIFO_size-1))/handles.Fout) ((tmp0*(N_window*(FIFO_size-1))-1)/handles.Fout) -0.1 Plot_Ylimit])
    title('t vs v (AinL)','Color', 'b') 
    xlabel(handles.axes_total(1),'time (s)')
    ylabel(handles.axes_total(1),'voltage (v)')
    hold(handles.axes_total(1),'on')
    plot(handles.axes_total(6),((((i-1)*(FIFO_size-1)):1:(i*(FIFO_size-1)-1))/handles.Fout),data_tmp_R)
    axis(handles.axes_total(6),[((tmp0-1)*(N_window*(FIFO_size-1))/handles.Fout) ((tmp0*(N_window*(FIFO_size-1))-1)/handles.Fout) -0.1 Plot_Ylimit])
    title('t vs v (AinR)','Color', 'b') 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
plot(handles.axes_total(3),((1:length(handles.data_L))/handles.Fout),handles.data_L)
title(handles.axes_total(3),'t vs v (TIA L)','Color', 'b') 
xlabel(handles.axes_total(3),'time (s)')
ylabel(handles.axes_total(3),'voltage (v)')
grid minor
plot(handles.axes_total(8),((1:length(handles.data_R))/handles.Fout),handles.data_R)
title(handles.axes_total(8),'t vs v (TIA R)','Color', 'b')
xlabel(handles.axes_total(8),'time (s)')
ylabel(handles.axes_total(8),'voltage (v)')
grid minor

plot(handles.axes_total(4),((1:length(handles.data_L))/handles.Fout),handles.current_L*(10^9))
title(handles.axes_total(4),'t vs i (TIA L)','Color', 'b')
xlabel(handles.axes_total(4),'time (s)')
ylabel(handles.axes_total(4),'current (nA)')
grid minor
plot(handles.axes_total(9),((1:length(handles.data_R))/handles.Fout),handles.current_R*(10^9))
title(handles.axes_total(9),'t vs i (TIA R)','Color', 'b')
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
fileID=fopen('DC_RESULT_CHL.txt','w');
fprintf(fileID,'%f\n', handles.data_L);
fclose(fileID);

fileID=fopen('DC_RESULT_CHR.txt','w');
fprintf(fileID,'%f\n', handles.data_R);
fclose(fileID);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
fileID=fopen('V2I_RESULT_CHL.txt','w');
fprintf(fileID,'%f\n', handles.data_L);
fclose(fileID);

fileID=fopen('V2I_RESULT_CHR.txt','w');
fprintf(fileID,'%f\n', handles.data_R);
fclose(fileID);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
data=[handles.data_L handles.data_R]; 
[m,n] = size(data); 
V_set=(5:1:28)'/10; 
%V_set=V_set.'; 
%Fs=16000; 
...
for k=1:n 
    ...
    [Code_tmp, p_tmp]=Code_To_Voltage(V_set,data(:,k),handles.Fout,Span_center,Span_range,Duration); 
    Code=[Code Code_tmp]; 
    p=[p p_tmp]; 
...
assignin('base','Slope_Left',handles.p_Left(1));
assignin('base','Offset_Left',handles.p_Left(2));
assignin('base','Slope_Right',handles.p_Right(1));
assignin('base','Offset_Right',handles.p_Right(2));

fileID = fopen('Code2Voltage.txt','w');
fprintf(fileID,'%12.6f %12.6f\r\n',handles.p_Left(1),handles.p_Right(1));
fprintf(fileID,'%12.6f %12.6f\r\n',handles.p_Left(2),handles.p_Right(2));
fclose(fileID);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
fileID=fopen('FSCV_VOLTAGE_CHL.txt','w');
fprintf(fileID,'%f\n', handles.data_L);
fclose(fileID);

fileID=fopen('FSCV_VOLTAGE_CHR.txt','w');
fprintf(fileID,'%f\n', handles.data_R);
fclose(fileID);

fileID=fopen('FSCV_CURRENT_CHL.txt','w');
fprintf(fileID,'%f\n', 10e9*(handles.current_L));
fclose(fileID);

fileID=fopen('FSCV_CURRENT_CHR.txt','w');
fprintf(fileID,'%f\n', 10e9*(handles.current_R));
fclose(fileID);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
str=inputdlg({'FSCV Period','FSCV ScanRate','FSCV Peak Voltage','FSCV Buttom Voltage','FSCV Vcom','TIA mode 1 to 4'},'Input dialog');
...
switch R_select
    case 1
        RfeedBack=[handles.RfeedBack_TIA1L handles.RfeedBack_TIA1R];
    case 2
        RfeedBack=[handles.RfeedBack_TIA2L handles.RfeedBack_TIA2R];
    case 3
        RfeedBack=[handles.RfeedBack_TIA1L handles.RfeedBack_TIA2R];
    case 4
        RfeedBack=[handles.RfeedBack_TIA2L handles.RfeedBack_TIA1R];        
    otherwise
        RfeedBack=[handles.RfeedBack_TIA1 handles.RfeedBack_TIA1];
end
%BTPIPEOUT
tolerance_range=0.9;
N_ok=5; 
...
data_tmp_L((FIFO_size-1),1) = 0;
data_tmp_R((FIFO_size-1),1) = 0;
current_tmp_L((FIFO_size-1),1) = 0;
current_tmp_R((FIFO_size-1),1) = 0;
data_cycle_L=[];
data_cycle_R=[];
...
for i= 1:cycle
    ...
    data_tmp_L = (data_tmp_L-handles.p_Left(2))/(handles.p_Left(1));
    handles.data_L=[handles.data_L; data_tmp_L];
    current_tmp_L=(data_tmp_L-Vcom)/(RfeedBack(1));
    handles.current_L=[handles.current_L; current_tmp_L];
    data_cycle_L=[data_cycle_L; data_tmp_L];
    data_tmp_R = quantizer_one_forth((double(epvalue_A1(1:2:(2*(FIFO_size-1)-1)))+256*double(epvalue_A1(2:2:(2*(FIFO_size-1)))))/(2^(16-ENOB)));
    data_tmp_R = (data_tmp_R-handles.p_Right(2))/(handles.p_Right(1));
    handles.data_R=[handles.data_R; data_tmp_R];
    current_tmp_R=(data_tmp_R-Vcom)/(RfeedBack(2));
    handles.current_R=[handles.current_R; current_tmp_R];
    data_cycle_R=[data_cycle_R; data_tmp_R];
    
    tmp0 = round(((i-1)-mod((i-1),N_window))/N_window)+1;
    plot(handles.axes_total(3),((((i-1)*(FIFO_size-1)):1:(i*(FIFO_size-1)-1))/handles.Fout),data_tmp_L)
    axis(handles.axes_total(3),[((tmp0-1)*(N_window*(FIFO_size-1))/handles.Fout) ((tmp0*(N_window*(FIFO_size-1))-1)/handles.Fout) -0.1 Plot_Ylimit])
    title(handles.axes_total(3),'t vs v (TIA L)','Color', 'b') 
    xlabel(handles.axes_total(3),'time (s)')
    ylabel(handles.axes_total(3),'voltage (v)')
    hold(handles.axes_total(3),'on')
    plot(handles.axes_total(8),((((i-1)*(FIFO_size-1)):1:(i*(FIFO_size-1)-1))/handles.Fout),data_tmp_R)
    axis(handles.axes_total(8),[((tmp0-1)*(N_window*(FIFO_size-1))/handles.Fout) ((tmp0*(N_window*(FIFO_size-1))-1)/handles.Fout) -0.1 Plot_Ylimit])
    title(handles.axes_total(8),'t vs v (TIA R)','Color', 'b')
    xlabel(handles.axes_total(8),'time (s)')
    ylabel(handles.axes_total(8),'voltage (v)')
    hold(handles.axes_total(8),'on')
    
    plot(handles.axes_total(4),((((i-1)*(FIFO_size-1)):1:(i*(FIFO_size-1)-1))/handles.Fout),current_tmp_L*(10^9))
    ...
    plot(handles.axes_total(9),((((i-1)*(FIFO_size-1)):1:(i*(FIFO_size-1)-1))/handles.Fout),current_tmp_R*(10^9))
    ...
    if mod(i,N_window)==0
        [data_FSCV_L, N_L]=FSCV_cycle_v2(data_cycle_L, handles.Fout, Period, ScanRate, Vr_H, Vr_L, tolerance_range, N_ok, handles.p_Left);
        [data_FSCV_R, N_R]=FSCV_cycle_v2(data_cycle_R, handles.Fout, Period, ScanRate, Vr_H, Vr_L, tolerance_range, N_ok, handles.p_Right);
        if length(data_FSCV_L)==0
            imp_L=zeros(1,(2*N_L+1));
        else
            imp_L=(((mean(data_FSCV_L')-0)/(1))-Vcom)/(RfeedBack(1));
        end
        if length(data_FSCV_R)==0
            imp_R=zeros(1,(2*N_R+1));
        else
            imp_R=(((mean(data_FSCV_R')-0)/(1))-Vcom)/(RfeedBack(2));
        end
        %handles.current_L=[handles.current_L; imp_L];
        %handles.current_R=[handles.current_R; imp_R];
        Xaxes_L=[(((0:(N_L-1))*((Vr_H-Vr_L)/N_L))+Vr_L) Vr_H ((((N_L-1):(-1):0)*((Vr_H-Vr_L)/N_L))+Vr_L)];
        Xaxes_R=[(((0:(N_R-1))*((Vr_H-Vr_L)/N_R))+Vr_L) Vr_H ((((N_R-1):(-1):0)*((Vr_H-Vr_L)/N_R))+Vr_L)];
        plot(handles.axes_total(5),Xaxes_L,imp_L*(10^9))
        axis(handles.axes_total(5),[Vr_L Vr_H -inf inf])
        title(handles.axes_total(5),'v vs i (TIA L)','Color', 'b')
        xlabel(handles.axes_total(5),'FSCV VCM (V)')
        ylabel(handles.axes_total(5),'Input Current (nA)')
        %legend(handles.axes_total(5),'')
        plot(handles.axes_total(10),Xaxes_R,imp_R*(10^9))
        axis(handles.axes_total(10),[Vr_L Vr_H -inf inf])
        title(handles.axes_total(10),'v vs i (TIA R)','Color', 'b')
...
[data_FSCV_L, N_L]=FSCV_cycle_v2(handles.data_L, handles.Fout, Period, ScanRate, Vr_H, Vr_L, tolerance_range, N_ok, handles.p_Left);
[data_FSCV_R, N_R]=FSCV_cycle_v2(handles.data_R, handles.Fout, Period, ScanRate, Vr_H, Vr_L, tolerance_range, N_ok, handles.p_Right);
FSCV_L_avg=mean(data_FSCV_L');
FSCV_R_avg=mean(data_FSCV_R');

switch R_select
    case 1
        handles.RfeedBack_TIA1L=(mean(FSCV_L_avg(X_set1)')-mean(FSCV_L_avg(X_set2)'))/(2*Cin_L*ScanRate);
        handles.RfeedBack_TIA1R=(mean(FSCV_R_avg(X_set1)')-mean(FSCV_R_avg(X_set2)'))/(2*Cin_R*ScanRate);
    case 2
        handles.RfeedBack_TIA2L=(mean(FSCV_L_avg(X_set1)')-mean(FSCV_L_avg(X_set2)'))/(2*Cin_L*ScanRate);
        handles.RfeedBack_TIA2R=(mean(FSCV_R_avg(X_set1)')-mean(FSCV_R_avg(X_set2)'))/(2*Cin_R*ScanRate);
    case 3
        handles.RfeedBack_TIA1L=(mean(FSCV_L_avg(X_set1)')-mean(FSCV_L_avg(X_set2)'))/(2*Cin_L*ScanRate);
        handles.RfeedBack_TIA2R=(mean(FSCV_R_avg(X_set1)')-mean(FSCV_R_avg(X_set2)'))/(2*Cin_R*ScanRate);
    case 4
...
assignin('base','TIA1L_Rf',handles.RfeedBack_TIA1L);
assignin('base','TIA1R_Rf',handles.RfeedBack_TIA1R);
assignin('base','TIA2L_Rf',handles.RfeedBack_TIA2L);
assignin('base','TIA2R_Rf',handles.RfeedBack_TIA2R);
...
for i= 1:cycle
    ...
    plot(handles.axes_total(2),((((i-1)*(FIFO_size-1)):1:(i*(FIFO_size-1)-1))/handles.Fout),data_tmp_L)
    ...
    title(handles.axes_total(2),'t vs code (AinL)','Color', 'b')
    xlabel(handles.axes_total(2),'time (s)')
    ylabel(handles.axes_total(2),'Code Value')
    hold(handles.axes_total(2),'on')
    plot(handles.axes_total(7),((((i-1)*(FIFO_size-1)):1:(i*(FIFO_size-1)-1))/handles.Fout),data_tmp_R)
    ...
    title(handles.axes_total(7),'t vs code (AinR)','Color', 'b')
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
plot(handles.axes_total(2),((1:length(handles.data_L))/handles.Fout),handles.data_L)
title(handles.axes_total(2),'t vs code (AinL)','Color', 'b')
xlabel(handles.axes_total(2),'time (s)')
ylabel(handles.axes_total(2),'Code Value')
grid minor
plot(handles.axes_total(7),((1:length(handles.data_R))/handles.Fout),handles.data_R)
title(handles.axes_total(7),'t vs code (AinR)','Color', 'b')
xlabel(handles.axes_total(7),'time (s)')
ylabel(handles.axes_total(7),'Code Value')
grid minor
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
plot(handles.axes_total(1),((1:length(handles.data_L))/handles.Fout),handles.data_L)
title(handles.axes_total(1),'t vs voltage (AinL)','Color', 'b')
xlabel(handles.axes_total(1),'time (s)')
ylabel(handles.axes_total(1),'voltage (v)')
grid minor
plot(handles.axes_total(6),((1:length(handles.data_R))/handles.Fout),handles.data_R)
title(handles.axes_total(6),'t vs voltage (AinR)','Color', 'b')
xlabel(handles.axes_total(6),'time (s)')
ylabel(handles.axes_total(6),'voltage (v)')
grid minor
