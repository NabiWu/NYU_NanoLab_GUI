...
for ii = 1:2
    ...
    tmp=subplot(4,3,1); 
    title("t vs v (AinL"+ii+")",'Color', 'b') 
    xlabel('time (s)') 
    ylabel('voltage (V)') 
    handles.axes_total=[handles.axes_total tmp]; 

    tmp=subplot(4,3,4); 
    title("t vs code (AinL"+ii+")",'Color', 'b') 
    xlabel('time (s)') 
    ylabel('Code Value (quarter bit)') 
    handles.axes_total=[handles.axes_total tmp]; 

    tmp=subplot(4,3,2); 
    title("t vs v (TIAXX L"+ii+")",'Color', 'b') 
    xlabel('time (s)') 
    ylabel('voltage (V)') 
    handles.axes_total=[handles.axes_total tmp]; 

    tmp=subplot(4,3,5); 
    title("t vs i (TIAXX L"+ii+")",'Color', 'b') 
    xlabel('time (s)') 
    ylabel('Current (nA)') 
    handles.axes_total=[handles.axes_total tmp]; 

    tmp=subplot(4,3,[3 6]); 
    title("v vs i (TIAXX L"+ii+")",'Color', 'b') 
    xlabel('Vcm voltage (v)') 
    ylabel('Current (nA)') 
    handles.axes_total=[handles.axes_total tmp]; 

    tmp=subplot(4,3,7); 
    title("t vs v (AinR"+ii+")",'Color', 'b') 
    xlabel('time (s)') 
    ylabel('voltage (V)') 
    handles.axes_total=[handles.axes_total tmp]; 

    tmp=subplot(4,3,10); 
    title("t vs code (AinR"+ii+")",'Color', 'b') 
    xlabel('time (s)') 
    ylabel('Code Value (quarter bit)') 
    handles.axes_total=[handles.axes_total tmp]; 

    tmp=subplot(4,3,8); 
    title("t vs v (TIAXX R"+ii+")",'Color', 'b') 
    xlabel('time (s)') 
    ylabel('voltage (V)') 
    handles.axes_total=[handles.axes_total tmp]; 

    tmp=subplot(4,3,11); 
    title("t vs i (TIAXX R"+ii+")",'Color', 'b') 
    xlabel('time (s)') 
    ylabel('Current (nA)') 
    handles.axes_total=[handles.axes_total tmp]; 

    tmp=subplot(4,3,[9 12]); 
    title("v vs i (TIAXX R"+ii+")",'Color', 'b') 
    xlabel('Vcm voltage (v)') 
    ylabel('Current (nA)') 
    handles.axes_total=[handles.axes_total tmp];
end
...
for i= 1:cycle
    ...
    axis(handles.axes_total(11),[((tmp0-1)*(N_window*(FIFO_size-1))/handles.Fout) ((tmp0*(N_window*(FIFO_size-1))-1)/handles.Fout) -0.1 Plot_Ylimit])
    ...
    axis(handles.axes_total(16),[((tmp0-1)*(N_window*(FIFO_size-1))/handles.Fout) ((tmp0*(N_window*(FIFO_size-1))-1)/handles.Fout) -0.1 Plot_Ylimit])
...
title(handles.axes_total(13),'t vs v (TIA L)','Color', 'b') 
xlabel(handles.axes_total(13),'time (s)')
ylabel(handles.axes_total(13),'voltage (v)')
grid minor
plot(handles.axes_total(18),((1:length(handles.data_R))/handles.Fout),handles.data_R)
title(handles.axes_total(18),'t vs v (TIA R)','Color', 'b')
...
for k=1:n2 
    ...
    [Code_tmp2, p_tmp2]=Code_To_Voltage(V_set,data(:,k),handles.Fout,Span_center,Span_range,Duration); 
...
for i= 1:cycle
    ...
    data_tmp_L2 = (data_tmp_L2-handles.p_Left2(2))/(handles.p_Left2(1));
    handles.data_L2=[handles.data_L2; data_tmp_L2];
    current_tmp_L2=(data_tmp_L2-Vcom)/(RfeedBack(1));
    ...
    data_tmp_R2 = (data_tmp_R2-handles.p_Right2(2))/(handles.p_Right2(1));
    handles.data_R2=[handles.data_R2; data_tmp_R2];
    current_tmp_R2=(data_tmp_R2-Vcom)/(RfeedBack(2));
    ...
    axis(handles.axes_total(13),[((tmp0-1)*(N_window*(FIFO_size-1))/handles.Fout) ((tmp0*(N_window*(FIFO_size-1))-1)/handles.Fout) -0.1 Plot_Ylimit])
    ...
    axis(handles.axes_total(18),[((tmp0-1)*(N_window*(FIFO_size-1))/handles.Fout) ((tmp0*(N_window*(FIFO_size-1))-1)/handles.Fout) -0.1 Plot_Ylimit])
    ...
    axis(handles.axes_total(14),[((tmp0-1)*(N_window*(FIFO_size-1))/handles.Fout) ((tmp0*(N_window*(FIFO_size-1))-1)/handles.Fout) -inf inf])
    title(handles.axes_total(14),'t vs i (TIA L2)','Color', 'b')
    xlabel(handles.axes_total(14),'time (s)')
    ylabel(handles.axes_total(14),'current (nA)')
    ...
    axis(handles.axes_total(19),[((tmp0-1)*(N_window*(FIFO_size-1))/handles.Fout) ((tmp0*(N_window*(FIFO_size-1))-1)/handles.Fout) -inf inf])
    title(handles.axes_total(19),'t vs i (TIA R2)','Color', 'b')
    xlabel(handles.axes_total(19),'time (s)')
    ylabel(handles.axes_total(19),'current (nA)')
...
switch R_select
    case 1
        handles.RfeedBack_TIA1L=(mean(FSCV_L1_avg(X_set1)')-mean(FSCV_L1_avg(X_set2)'))/(2*Cin_L*ScanRate);
        handles.RfeedBack_TIA1R=(mean(FSCV_R1_avg(X_set1)')-mean(FSCV_R1_avg(X_set2)'))/(2*Cin_R*ScanRate);
    case 2
        handles.RfeedBack_TIA2L=(mean(FSCV_L1_avg(X_set1)')-mean(FSCV_L1_avg(X_set2)'))/(2*Cin_L*ScanRate);
        handles.RfeedBack_TIA2R=(mean(FSCV_R1_avg(X_set1)')-mean(FSCV_R1_avg(X_set2)'))/(2*Cin_R*ScanRate);
    case 3
        handles.RfeedBack_TIA1L=(mean(FSCV_L1_avg(X_set1)')-mean(FSCV_L1_avg(X_set2)'))/(2*Cin_L*ScanRate);
        handles.RfeedBack_TIA2R=(mean(FSCV_R1_avg(X_set1)')-mean(FSCV_R1_avg(X_set2)'))/(2*Cin_R*ScanRate);
    case 4
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
plot(handles.axes_total(2),((1:length(handles.data_L))/handles.Fout),handles.data_L1)
title(handles.axes_total(2),'t vs code (AinL1)','Color', 'b')
xlabel(handles.axes_total(2),'time (s)')
ylabel(handles.axes_total(2),'Code Value')
grid minor
plot(handles.axes_total(7),((1:length(handles.data_R))/handles.Fout),handles.data_R1)
title(handles.axes_total(7),'t vs code (AinR1)','Color', 'b')
xlabel(handles.axes_total(7),'time (s)')
ylabel(handles.axes_total(7),'Code Value')
grid minor

plot(handles.axes_total(12),((1:length(handles.data_L))/handles.Fout),handles.data_L2)
title(handles.axes_total(12),'t vs code (AinL2)','Color', 'b')
xlabel(handles.axes_total(12),'time (s)')
ylabel(handles.axes_total(12),'Code Value')
grid minor
plot(handles.axes_total(17),((1:length(handles.data_R))/handles.Fout),handles.data_R2)
title(handles.axes_total(17),'t vs code (AinR2)','Color', 'b')
xlabel(handles.axes_total(17),'time (s)')
ylabel(handles.axes_total(17),'Code Value')
grid minor
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
plot(handles.axes_total(1),((1:length(handles.data_L))/handles.Fout),handles.data_L1)
title(handles.axes_total(1),'t vs voltage (AinL1)','Color', 'b')
xlabel(handles.axes_total(1),'time (s)')
ylabel(handles.axes_total(1),'voltage (v)')
grid minor
plot(handles.axes_total(6),((1:length(handles.data_R))/handles.Fout),handles.data_R1)
title(handles.axes_total(6),'t vs voltage (AinR1)','Color', 'b')
xlabel(handles.axes_total(6),'time (s)')
ylabel(handles.axes_total(6),'voltage (v)')
grid minor

plot(handles.axes_total(11),((1:length(handles.data_L))/handles.Fout),handles.data_L2)
title(handles.axes_total(11),'t vs voltage (AinL2)','Color', 'b')
xlabel(handles.axes_total(11),'time (s)')
ylabel(handles.axes_total(11),'voltage (v)')
grid minor
plot(handles.axes_total(16),((1:length(handles.data_R))/handles.Fout),handles.data_R2)
