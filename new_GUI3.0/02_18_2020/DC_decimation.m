...
%      stop.  2All inputs are passed to DC_decimation_OpeningFcn via varargin.
%
...
% % 13
% handles.p_Left1=[-2193.48; 3836.5];
% handles.p_Right1=[-2193.48; 3836.5];
% handles.p_Left2=[-2193.48; 3836.5];
% handles.p_Right2=[-2193.48; 3836.5];
% % 16
...
for i= 1:cycle
    ...
    data_tmp_L1 = floor((double(epvalue_A0(1:4:(4*(FIFO_size-1)-1)))+256*double(epvalue_A0(2:4:(4*(FIFO_size-1)))))/(2^(16-ENOB)));
    data_tmp_L1=(data_tmp_L1-handles.p_Left1(2))/handles.p_Left1(1);
    handles.data_L1=[handles.data_L1; data_tmp_L1];

    data_tmp_R1 = floor((double(epvalue_A1(1:4:(4*(FIFO_size-1)-1)))+256*double(epvalue_A1(2:4:(4*(FIFO_size-1)))))/(2^(16-ENOB)));
    data_tmp_R1=(data_tmp_R1-handles.p_Right1(2))/handles.p_Right1(1);
    handles.data_R1=[handles.data_R1; data_tmp_R1];
    
    data_tmp_L2 = floor((double(epvalue_A2(1:4:(4*(FIFO_size-1)-1)))+256*double(epvalue_A2(2:4:(4*(FIFO_size-1)))))/(2^(16-ENOB)));
    data_tmp_L2=(data_tmp_L2-handles.p_Left2(2))/handles.p_Left2(1);
    handles.data_L2=[handles.data_L2; data_tmp_L2];

    data_tmp_R2 = floor((double(epvalue_A3(1:4:(4*(FIFO_size-1)-1)))+256*double(epvalue_A3(2:4:(4*(FIFO_size-1)))))/(2^(16-ENOB)));
...
%     disp(length(epvalue_A2));
    ...
    data_tmp_R1 = (data_tmp_R1-handles.p_Right1(2))/(handles.p_Right1(1));
    handles.data_R1=[handles.data_R1; data_tmp_R1];
    current_tmp_R1=(data_tmp_R2-Vcom)/(RfeedBack(2));
